...
async def run():
    ...
    print_health_task = asyncio.ensure_future(print_health_task(drone))
    print_coords_task = asyncio.ensure_future(read_coords(conn))
...
async def print_health(drone):
    ...
    print(battery_percentage + GPS + pos)    
