...
void switchDir(String motor) {
  ...
  digitalWrite(dirPin[motorNum], dir[motorNum]);
}
...
void actuate(String motor, float distance) {
  ...
  float totalPulses_temp = abs(distance) / leadVal[motorNum] * (float) pulsePerRev[motorNum];
  unsigned long totalPulses = 0;
  for (int i = 0; i < totalPulses_temp; i++) {
    totalPulses++;
  }
  unsigned long totalCount = 0;
  for (int i = 0; i < totalPulses * gearRatio[motorNum]; i++) {
    pulse(motor);
  }
...
void startEndPointsExperiment() {
  ...
  for (int i = 0; i < numZPoints; i++) {
    ...
    if (i == 0) {
      ...
      thruster.writeMicroseconds(sig);
      delay(10000);
